...
public class pairedT_Test {
    private static final double[][] tTabulatedValue ={{0.000, 1.000, 1.376, 1.963, 3.078, 6.314, 12.71, 31.82, 63.66, 318.31, 636.62},
        ...
        {0.000 ,0.683, 0.854 ,1.055 ,1.310 ,1.697 ,2.042 ,2.457 ,2.750 ,3.385, 3.646}};//used to store tValue[deg Of Freedom][level of significance]
    private final HashMap<Double,Integer> losToIndex=new HashMap<>();//maps level of significance to Index, to extract tabulated values in constant time
    pairedT_Test()
    {
        losToIndex.put(50.0,0);
        losToIndex.put(25.0,1);
        losToIndex.put(20.0,2);
        losToIndex.put(15.0,3);
        losToIndex.put(10.0,4);
        losToIndex.put(5.0,5);
        losToIndex.put(2.5,6);
        losToIndex.put(1.0,7);
        losToIndex.put(0.5,8);
        losToIndex.put(0.01,9);
        losToIndex.put(0.005,10);
    }

    public void tTest(double D, double sqD,int N)
    {
        int degOfFreedom=N-1;
        double tCalculatedValue=(D/N)*Math.sqrt(N)/Math.sqrt((sqD-(Math.pow(D,2)/N))/(N-1));
        System.out.println("Mean of differences is: "+D/N);
        System.out.println("Calculated T-value is: "+tCalculatedValue);
        System.out.println("ENTER LEVEL OF SIGNIFICANCE");
        Scanner sc=new Scanner(System.in);
        double l=sc.nextDouble();
        double tabValue=tTabulatedValue[degOfFreedom-1][losToIndex.get(l)];//Extracts tabulated Value in constant time with the help of hashMap. DoF-1 for index.
        System.out.println("Tabulated T-value is: "+tabValue);
        if(tCalculatedValue>tabValue)
            System.out.println("WE REJECT THE NULL HYPOTHESIS");
        else
            System.out.println("NULL HYPOTHESIS STANDS");
    }
    public double sumOfDiff(int[] arr, int[] brr)
    {
        double diff=0;
        ...
        System.out.println("Sum of differences is: "+diff);
        return diff;
    }
    public double sumOfSquaredDiff(int[] arr, int[] brr)
    {
        double diff=0;
        for(int i=0;i<arr.length;i++)
        {
            diff+=Math.pow(arr[i]-brr[i],2);
        }
        System.out.println("Sum of squared differences is: "+diff);
        return diff;
    }
}
